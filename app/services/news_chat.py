"""⑤ 챗봇용 외부 뉴스 수집 — 담당자가 보고서를 읽다 막히는 배경지식을 채우는 용도.

⑥ 동향 인사이트가 쓰는 `news.py` 와는 목적이 다르다.
이쪽은 "질문 한 건에 붙일 근거 기사 5건"이라 네이버 API 를 먼저 쓰고,
`news.py` 는 "보고서 키워드로 모은 넓은 후보군"이라 RSS 를 통째로 채점한다.
두 모듈이 한 파일에 병합돼 있던 것을 분리했다.
